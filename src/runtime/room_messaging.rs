...
fn tool() -> Value {
    let body = json!({"type":"string","minLength":1,"description":"Write the shared answer as Markdown. Wrap table, column, field, function, command, and path identifiers in backticks; use fenced code blocks for multi-line snippets. Preserve Markdown formatting when publishing."});
    json!({"name":"send_room_message","description":"Publish an explicit shared message in the current Room. Use targets=[] to answer the Room without waking agents; name Room agents only when requesting their attention. Set reply_to to the message being answered. Private runtime output is not published. Use request_id to safely retry the same message. For lifecycle-bearing delegation only, add work with action=create and title (optional goal), or action=bind and work_id. Work requires one owner target and request_id; omit work for ordinary chat.","inputSchema":{"type":"object","properties":{"targets":{"type":"array","minItems":0,"items":{"type":"string","minLength":1}},"body":body,"reply_to":{"type":"string"},"request_id":{"type":"string","minLength":1},"work":{"oneOf":[{"type":"object","properties":{"action":{"const":"create"},"title":{"type":"string","minLength":1},"goal":{"type":"string","minLength":1}},"required":["action","title"],"additionalProperties":false},{"type":"object","properties":{"action":{"const":"bind"},"work_id":{"type":"string","minLength":1}},"required":["action","work_id"],"additionalProperties":false}]}},"required":["targets","body"],"additionalProperties":false}})
}
