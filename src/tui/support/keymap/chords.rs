...
use crate::tui::support::key_hint::ctrl;
...
const FIRST_DISPATCH_FUNCTION_KEY: u8 = crate::tui::support::tui_keymap::MAX_FUNCTION_KEY + 1;
...
or a two-stroke chord such as `ctrl-x ctrl-t`.",
        ...
        let g = crate::tui::support::key_hint::plain(KeyCode::Char('g'));
    ...
    pub(crate) fn primary_hint(
        ...
        bindings: &[KeyBinding],
    ) -> Option<crate::tui::support::key_hint::ShortcutHint> {
        if let Some(spec) = self
            .configured_specs(action)
            .and_then(|specs| specs.first())
        {
            return if let Some((prefix, completion)) = spec.split_once(' ') {
                Some(crate::tui::support::key_hint::ShortcutHint::Chord {
                    prefix: parse_keybinding(prefix)?,
                    completion: parse_keybinding(completion)?,
                })
            } else {
                parse_keybinding(spec).map(crate::tui::support::key_hint::ShortcutHint::Single)
            };
        }

        super::primary_binding(bindings)
            .map(crate::tui::support::key_hint::ShortcutHint::Single)
            .or_else(|| {
                self.bindings
                    .iter()
...
impl KeyChordMatcher {
    ...
    pub(crate) fn advance(
        ...
        if let Some(pending) = self.pending.take() {
            if crate::tui::support::key_hint::plain(KeyCode::Esc).is_press(key_event) {
...
fn chord_stroke_matches(binding: KeyBinding, key_event: KeyEvent) -> bool {
    let ctrl_slash = crate::tui::support::key_hint::ctrl(KeyCode::Char('/'));
    let ctrl_seven = crate::tui::support::key_hint::ctrl(KeyCode::Char('7'));
...
pub(crate) fn normalize_chord_binding(binding: KeyBinding) -> KeyBinding {
    ...
    if binding.parts() == crate::tui::support::key_hint::ctrl(KeyCode::Char('7')).parts() {
        crate::tui::support::key_hint::ctrl(KeyCode::Char('/'))
    } else {
        binding
    }
}
...
fn validate_binding_shape(binding: &RuntimeChordBinding) -> Result<(), String> {
    ...
    if matches!(prefix_key, KeyCode::Char(_))
        && crate::tui::support::key_hint::is_altgr(prefix_modifiers)
...
Windows. Choose a different chord and retry.",
    ...
    if matches!(prefix_key, KeyCode::Char(_))
        && !crate::tui::support::key_hint::has_ctrl_or_alt(prefix_modifiers)
        && !binding.action.context.allows_plain_chord_prefix()
...
fn validate_reserved_strokes(binding: &RuntimeChordBinding) -> Result<(), String> {
    ...
    #[cfg(unix)]
    if strokes.contains(&crate::tui::support::key_hint::ctrl(KeyCode::Char('z')).parts()) {
        return Err(format!(
