...
use crate::tui::support::render::line_utils::push_owned_lines;
...
/// Checks for `scheme://host` patterns so wrapping does not break a URL in half.
///
/// Well-known network schemes additionally need a non-empty host; custom schemes are taken at face
/// value once the scheme itself is well formed.
///
/// ponytail: `has_valid_scheme_prefix` plus a host check, instead of `url::Url::parse` and the IDNA
/// tree behind it. This only decides where a line wraps, so a misjudged exotic URL costs one ugly
/// break, not correctness.
fn is_absolute_url_like(token: &str) -> bool {
    let Some((scheme, rest)) = token.split_once("://") else {
        return false;
    };
    if !has_valid_scheme_prefix(token) {
        return false;
    }
    if matches!(
        ...
        return !url_host(rest).is_empty();
    }
    true
}

/// Extracts the host from the part of a URL that follows `://`.
