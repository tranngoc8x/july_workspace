//! Keymap schema and canonical key-spec normalization.
//!
//! Codex reads a `[tui.keymap]` section out of `config.toml` and overlays it on the built-in
//! bindings. July has no such section, so these types survive only as the shape the ported keymap
//! resolver expects: it is always handed a `TuiKeymap::default()`, every action is `None`, and the
//! built-in defaults always win.
//!
//! What still earns its keep is [`normalize_keybinding_spec`] and the alias table under it, which
//! turn key names like `escape` or `pageup` into the canonical forms the resolver parses.
//!
//! ponytail: serde and schemars derives stripped along with the config contract. Put them back, and
//! this module is a working config schema again.
