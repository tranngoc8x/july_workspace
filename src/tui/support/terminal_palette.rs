use crate::tui::support::color::perceptual_distance;
use crate::tui::support::terminal_detection::TerminalName;
use crate::tui::support::terminal_detection::terminal_info;
...
/// Reads the color depth the terminal advertises through its environment.
///
/// ponytail: env sniffing instead of the `supports_color` crate - the variables below are what that
/// crate reads anyway, and getting it wrong only costs color fidelity. Add the crate if a terminal
/// turns up that needs its tty probing.
pub fn stdout_color_level() -> StdoutColorLevel {
    color_level_from_env(|key| std::env::var(key).ok())
}

fn color_level_from_env(var: impl Fn(&str) -> Option<String>) -> StdoutColorLevel {
    let value = |key: &str| var(key).filter(|value| !value.is_empty());

    if value("NO_COLOR").is_some() {
        return StdoutColorLevel::Unknown;
    }
    let colorterm = value("COLORTERM").unwrap_or_default().to_ascii_lowercase();
    if colorterm == "truecolor" || colorterm == "24bit" {
        return StdoutColorLevel::TrueColor;
    }
    let Some(term) = value("TERM") else {
        return StdoutColorLevel::Unknown;
    };
...
/// The terminal's own foreground and background, when something has reported them.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
static DEFAULT_COLORS: std::sync::OnceLock<std::sync::Mutex<Option<DefaultColors>>> =
    std::sync::OnceLock::new();

fn default_colors_cell() -> &'static std::sync::Mutex<Option<DefaultColors>> {
...
pub fn set_default_colors(colors: Option<DefaultColors>) {
    if let Ok(mut cell) = default_colors_cell().lock() {
        *cell = colors;
    }
}
...
#[cfg(test)]
pub(crate) fn with_test_default_colors<T>(colors: DefaultColors, render: impl FnOnce() -> T) -> T {
    TEST_DEFAULT_COLORS.with(|override_colors| {
        let previous = override_colors.replace(Some(colors));
        let result = render();
