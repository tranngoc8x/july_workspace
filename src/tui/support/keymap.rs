...
use crate::tui::support::tui_keymap::KeybindingsSpec;
use crate::tui::support::tui_keymap::MAX_FUNCTION_KEY;
use crate::tui::support::tui_keymap::TuiKeymap;
    ...
    fn validate_conflicts(&self) -> Result<(), String> {
        ...
        if self.app.open_agents.iter().any(|binding| {
            matches!(binding.parts(), (KeyCode::Char(_), modifiers)
                if crate::tui::support::key_hint::is_altgr(modifiers))
        }) {
            return Err(
...
/// Whether a user-configured binding already claims `alias` on a main-surface input path.
///
/// ponytail: always false. July resolves the keymap from `TuiKeymap::default()` and never reads a
/// `[tui.keymap]` config section, so no action can be configured and no alias can collide. Restore
/// Codex's reflection over the config structs here if July ever makes keybindings configurable.
fn configured_main_surface_alias_is_used(_keymap: &TuiKeymap, _alias: &str) -> bool {
    false
}

/// Whether one keymap context configures `alias` for any of its actions.
///
/// See [`configured_main_surface_alias_is_used`]: with no config source, nothing is configured.
fn configured_context_alias_is_used<T>(_context: &T, _alias: &str) -> bool {
    false
}
...
mod tests {
    use super::*;
    use crate::tui::support::tui_keymap::KeybindingSpec;
    ...
    /// July always resolves from `TuiKeymap::default()`, so this is the only resolution path that
    /// can ever run. `RuntimeKeymap::defaults()` panics on failure, so assert it resolves cleanly
    /// rather than discovering it at startup.
    #[test]
    fn built_in_defaults_resolve_without_conflicts() {
        assert!(
            RuntimeKeymap::from_config(&TuiKeymap::default()).is_ok(),
            "built-in keymap defaults must resolve: {:?}",
            RuntimeKeymap::from_config(&TuiKeymap::default()).err()
        );
    }
