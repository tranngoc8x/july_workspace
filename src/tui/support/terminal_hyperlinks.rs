...
use crate::tui::support::line_truncation::line_width;
use crate::tui::support::render::line_utils::line_to_borrowed;
use crate::tui::support::render::line_utils::line_to_static;
use crate::tui::support::width::display_width;
use crate::tui::support::wrapping::RtOptions;
use crate::tui::support::wrapping::adaptive_wrap_line;
...
/// Accepts a destination only if it is an `http(s)` URL with a host, after control bytes are gone.
///
/// The escape-sequence safety comes from [`sanitized_destination`], which drops every control
/// character (so no `ESC` or `BEL` can close the OSC 8 payload early) and caps the length. This
/// check is the second gate: it keeps non-web schemes such as `file:` or `javascript:` out of the
/// terminal's clickable links.
///
/// ponytail: hand-rolled scheme/host split rather than the `url` crate, which drags the whole IDNA
/// tree in for two field reads. Swap in `Url::parse` if a destination ever needs real parsing.
pub(crate) fn web_destination(destination: &str) -> Option<String> {
    let safe_destination = sanitized_destination(destination)?;
    let rest = strip_web_scheme(&safe_destination)?;
    let authority = rest
        .split(['/', '?', '#'])
        .next()
        .unwrap_or_default()
    ...
    (!host.is_empty()).then_some(safe_destination)
}
