...
/// Position of the newest record to include in a bounded persistent-history lookup.
///
/// Opaque to the composer: it hands the cursor back out with a lookup request and receives a
/// continuation cursor in the response.
///
/// ponytail: Codex also carries a byte anchor so the log reader can resume a backward scan without
/// rescanning the file prefix. That belongs with the reader, and July has none yet, so only the row
/// offset survives. Add the anchor alongside whatever reads the prompt log.
...
impl HistoryBatchCursor {
    /// Creates a cursor ending at the given absolute history offset.
...
/// One absolute history offset covered by a bounded lookup; `entry` is `None` for a malformed row.
...
/// Persistent-history data routed back to the composer that requested it.
#[derive(Debug, Clone, PartialEq)]
...
impl HistoryEntry {
    ...
    ///
    /// ponytail: Codex re-parses `$tool` and `@plugin` links out of the stored text here to rebuild
    /// mention bindings. July stores plain prompts with plain `@agent` mentions, so there is nothing
    /// to decode. Bring back a decoder if July ever persists a mention link syntax.
...
impl ChatComposerHistory {
    ...
    pub fn set_metadata(&mut self, context: ContextId, log_id: u64, entry_count: usize) {
        let had_configured_thread = self.context.replace(context).is_some();
    ...
    pub fn navigate_up(&mut self, app_event_tx: &PaneEventSender) -> Option<HistoryEntry> {
        self.search = None;
    ...
    pub fn navigate_down(&mut self, app_event_tx: &PaneEventSender) -> Option<HistoryEntry> {
        self.search = None;
        let total_entries = self.persistent_entry_count + self.local_history.len();
        if total_entries == 0 {
            return None;
        ...
        while offset < total_entries {
                ...
                && offset < self.persistent_entry_count
            {
                    ...
                    (self.context.clone(), self.persistent_log_id)
                {
                    ...
                    app_event_tx.send(PaneEvent::LookupHistoryEntry {
                        context,
                        offset,
                        log_id,
                    });
