...
use crate::tui::bottom_pane::request_user_input::render::render_rows_bottom_aligned;
use crate::tui::bottom_pane::request_user_input::render::truncate_line_word_boundary_with_ellipsis;
...
impl Renderable for AsyncQuestions {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph::new(sections.question_lines.join("\n"))
            .style(crate::tui::support::style::accent_style())
            .bold()
...
impl AsyncQuestions {
    pub(super) fn footer_lines(
        ...
        if let Some(key) = self.keymap.primary_hint(context, action)
            && (self.focus_is_notes()
                || !matches!(key, crate::tui::support::key_hint::ShortcutHint::Single(binding)
                        ...
                        || self.keymap.chat.edit_queued_message.contains(&binding)))
        {
            tips.push(
                Span::styled(
                    ...
                    crate::tui::support::style::accent_style(),
                )
                .bold(),
            );
        }
