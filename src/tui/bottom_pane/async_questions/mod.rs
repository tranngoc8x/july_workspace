//! Inline editing for questions an agent asks mid-turn.
//!
//! A question arrives with a title and optionally a list of options. It is drafted inline rather
//! than in a modal, so it never steals focus: arrival and expiry leave the composer alone, and only
//! a locally accepted submission removes the question.

/// One question an agent is waiting on an answer for.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct AsyncUserInputQuestion {
    /// What is being asked.
...
/// Frames a submitted answer with the question it answers, so the agent has the context back.
///
/// The question is capped so a long model-authored prompt cannot crowd out the answer.
pub(crate) fn answered_question_framing(question: &str) -> String {
...
pub(crate) struct AsyncQuestions {
    ...
    pub(crate) next_hint: Option<crate::tui::support::key_hint::ShortcutHint>,
